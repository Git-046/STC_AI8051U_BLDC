...
#define PWMA_CARRIER_FREQ       20000       //PWMA的载波频率
#define PWMB_SOURCE_FREQUENCY   144000000   //PWMB的时钟源频率144MHz
//========================================================================
//                               PWM设置
//========================================================================
...
#define PWMB_UIEA_Enable()          PWMB_IER |= 0x01        //1：允许更新中断
#define PWMB_UIEA_Disable()         PWMB_IER &= ~0x01       //0：禁止更新中断
#define PWMB_CC5IE_Enable()         PWMB_IER |= 0x02        //1：允许捕获/比较中断
#define PWMB_CC5IE_Disable()        PWMB_IER &= ~0x02       //0：禁止捕获/比较中断
#define PWMB_CC6IE_Enable()         PWMB_IER |= 0x04        //1：允许捕获/比较中断
#define PWMB_CC6IE_Disable()        PWMB_IER &= ~0x04       //0：禁止捕获/比较中断
#define PWMB_CC7IE_Enable()         PWMB_IER |= 0x08        //1：允许捕获/比较中断
#define PWMB_CC7IE_Disable()        PWMB_IER &= ~0x08       //0：禁止捕获/比较中断
#define PWMB_CC8IE_Enable()         PWMB_IER |= 0x10        //1：允许捕获/比较中断
#define PWMB_CC8IE_Disable()        PWMB_IER &= ~0x10       //0：禁止捕获/比较中断
#define PWMB_COMIEA_Enable()        PWMB_IER |= 0x20        //1：允许COM中断
#define PWMB_COMIEA_Disable()       PWMB_IER &= ~0x20       //0：禁止COM中断
#define PWMB_TIEA_Enable()          PWMB_IER |= 0x40        //1：允许触发中断
#define PWMB_TIEA_Disable()         PWMB_IER &= ~0x40       //0：禁止触发中断
#define PWMB_BIEA_Enable()          PWMB_IER |= 0x80        //1：允许刹车中断
#define PWMB_BIEA_Disable()         PWMB_IER &= ~0x80       //0：禁止刹车中断
...
#define CCAS_IUTPUT_TI1FP1           0x01        //输入，IC1 映射在 TI1FP1 上	//PWM1P
#define CCAS_IUTPUT_TI2FP1           0x02        //输入，IC1 映射在 TI1FP2 上	//PWM1P
#define CCAS_IUTPUT_TI2FP2           0x01        //输入，IC2 映射在 TI2FP2 上	//PWM2P
#define CCAS_IUTPUT_TI1FP2           0x02        //输入，IC2 映射在 TI1FP2 上	//PWM2P
#define CCAS_IUTPUT_TI3FP3           0x01        //输入，IC3 映射在 TI3FP3 上	//PWM3P
#define CCAS_IUTPUT_TI4FP3           0x02        //输入，IC3 映射在 TI4FP3 上	//PWM3P
#define CCAS_IUTPUT_TI4FP4           0x01        //输入，IC4 映射在 TI4FP4 上	//PWM4P
#define CCAS_IUTPUT_TI4FP3           0x02        //输入，IC4 映射在 TI4FP3 上	//PWM4P
#define CCAS_IUTPUT_TRC              0x03        //输入，IC1/IC2/IC3/IC4 映射在 TRC 上

#define CCBS_OUTPUT                  0x00        //输出
#define CCBS_IUTPUT_TI5FP5           0x01        //输入，IC5 映射在 TI5FP5 上	//PWM5P
#define CCBS_IUTPUT_TI6FP5           0x02        //输入，IC5 映射在 TI6FP5 上	//PWM5P
#define CCBS_IUTPUT_TI6FP6           0x01        //输入，IC6 映射在 TI5FP5 上	//PWM6P
#define CCBS_IUTPUT_TI6FP5           0x02        //输入，IC6 映射在 TI6FP5 上	//PWM6P
#define CCBS_IUTPUT_TI7FP7           0x01        //输入，IC7 映射在 TI7FP7 上	//PWM7P
#define CCBS_IUTPUT_TI7FP8           0x02        //输入，IC7 映射在 TI7FP8 上	//PWM7P
#define CCBS_IUTPUT_TI8FP8           0x01        //输入，IC8 映射在 TI8FP8 上	//PWM8P
#define CCBS_IUTPUT_TI8FP7           0x02        //输入，IC8 映射在 TI8FP7 上	//PWM8P
...
#define PWM_ALIGN_CENTER_1	0x01
#define PWM_ALIGN_CENTER_2	0x02
#define PWM_ALIGN_CENTER_3	0x03

typedef struct
{ 
	u8    PWM_Output_Channel_Select;	/*	PWM输出通道选择,ENO1P,ENO1N,ENO2P,ENO2N,ENO3P,ENO3N,ENO4P,ENO4N,
															ENO5P,ENO6P,ENO7P,ENO8P		*/
	
	u8    PWM_In_Out_Mode;		/*  PWM信号映射选择： 
											输出：CCAS_OUTPUT; 输入:CCAS_IUTPUT_TI1FP1、CCAS_IUTPUT_TI2FP1、CCAS_IUTPUT_TRC
											输出：CCBS_OUTPUT; 输入:CCBS_IUTPUT_TI1FP1、CCBS_IUTPUT_TI2FP1、CCBS_IUTPUT_TRC  */
	
	/*如果PWM_Capture_Channel_Select选择输出：CCnS_OUTPUT,则可以选择输出PWM信号的Pin脚：*/
	char*    PWM_IO_Pin_Select; 	/*	PWM通道引脚可供选择:  */  //互补通道只能P0.0+P0.1或P1.0+P1.1这样两两对应选择
								/*	|					寄存器PWMA_PS				  |		*/
								/*	PWM1P: P0.0  P1.0  P2.0		PWM1N: P0.1  P1.1  P2.1		*/   
								/*	PWM2P: P0.2  P1.2  P2.2		PWM2N: P0.3  P1.3  P2.3		*/    
								/*	PWM3P: P0.4  P1.4  P2.4		PWM3N: P0.5  P1.5  P2.5		*/
								/*	PWM4P: P0.6  P1.6  P2.6		PWM4N: P0.7  P1.7  P2.7		*/
	...
	u8    PWM_Capture_Edge_or_Valid_Level;	//输出比较(0 高电平有效  1 低电平有效)  输入捕获(0 捕获上升沿  1 捕获下降沿)
	/* PWMx_CCMR */
	u8    PWM_EXTI_Clear_OC_Ref_EN;	//PWMETI引脚外部事件清通道输出信号(OCnREF)使能位 (0 不受影响  1 检测到ETRF输入高电平则OCnREF=0)
    u8    PWM_OC_Mode;				//捕获/比较 模式,CCMRn_FREEZE,CCMRn_MATCH_VALID,CCMRn_MATCH_INVALID,CCMRn_ROLLOVER,CCMRn_FORCE_INVALID,CCMRn_FORCE_VALID,CCMRn_PWM_MODE1,CCMRn_PWM_MODE2
	u8    PWM_OC_PreLoad_EN;		//输出比较的预装载使能, ENABLE,DISABLE
	u8    PWM_Fast_EN;				//输出比较快速功能使能, ENABLE,DISABLE
//	u8    PWM_In_Out_Mode;
	
	/* PWMx_CR1 组 */
	u8    PWM_ARR_PreLoad_EN;	//预装载使能,	ENABLE,DISABLE												
    u8	  PWM_Align;			//对齐模式  0x00边沿对齐模式，0x01中央对齐模式1，0x02中央对其模式2，0x03中央对齐模式3
	u8    PWM_Couter_Dir;		//计数器计数方向(0 向上计数   1 向下计数)
	u8    PWM_Single_Pulse_Mode;//单脉冲模式(0 发生更新事件时，计数器不停止  1 发生下一次更新事件时，清除CEN位，计数器停止)
	u8    PWM_Update_Source;	//更新请求源(0 寄存器更新、软件设置、时钟触发    1 寄存器更新)
	u8    PWM_Update_Dis;		//更新禁止使能位(0 允许更新事件    1 禁止更新事件)
	u8    PWM_Couter_EN;		//计数器使能位(0 禁止计数器    1 使能计数器)
	
	/* 组 */
	u16   PWM_Arr;					//PWM自动重装载值
	u16   PWM_Pscr;					//PWM时钟源预分频
    u8    PWM_DeadTime;				//死区发生器设置, 0~255
	
	u16   PWM_Duty;					//占空比, 0~Period
	u16   PWM_Set_Duty;		//占空比设置 0.0% ~ 100.0%
	u32   PWM_Frequency;	//PWM频率 0 ~ System_Clock
    
	u8    PWM_CCP_PreLoad_EN;		//捕获比较的预装载使能位, ENABLE,DISABLE
    u8    PWM_BrakeEnable;			//刹车输入使能位,	ENABLE,DISABLE
    u8    PWM_CEN_Enable;			//使能计数器, ENABLE,DISABLE
    u8    PWM_MainOutEnable;		//主输出使能位, ENABLE,DISABLE
    
    u8    PWM_Update_Interrupt_EN;  //更新中断使能位
    u8    PWM_CC_Interrupt_EN;      //捕获/比较中断使能位
} PWMx_InitDefine; 
...
typedef struct
{ 
//  u8    PWM_OC_Mode;             //模式,   CCMRn_FREEZE,CCMRn_MATCH_VALID,CCMRn_MATCH_INVALID,CCMRn_ROLLOVER,CCMRn_FORCE_INVALID,CCMRn_FORCE_VALID,CCMRn_PWM_MODE1,CCMRn_PWM_MODE2
    u16   PWM_Arr;           //周期时间,   0~65535
    u8    PWM_DeadTime;         //死区发生器设置, 0~255
    u8    PWM_Output_Channel_Select;//输出通道选择,    ENO1P,ENO1N,ENO2P,ENO2N,ENO3P,ENO3N,ENO4P,ENO4N / ENO5P,ENO6P,ENO7P,ENO8P
    u8    PWM_CEN_Enable;       //使能计数器, ENABLE,DISABLE
