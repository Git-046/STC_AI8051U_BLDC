...
#define 	ADC_PowerOn(n)			(n==0?(ADC_POWER = 0):(ADC_POWER = 1))	/* ADC电源开关 */
 #define 	ADC_ALIGN_MODE(n)		(n==0?(RESFMT = 0):(RESFMT = 1))		/* ADC转换结果格式控制, 左对齐/右对齐 */
...
typedef struct
{
	...
	u8	ADC_Align_Mode;		//ADC结果调整,	ADC_LEFT_JUSTIFIED,ADC_RIGHT_JUSTIFIED
	...
	char* ADC_Channel_Pin;	//ADC 通道引脚选择
	u8  ADC_PWMA_Triggle_EN; //PWMA更新触发ADC采样使能位
	u8  ADC_PWMB_Triggle_EN; //PWMA更新触发ADC采样使能位
} ADC_InitTypeDef;
