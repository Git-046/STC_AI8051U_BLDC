...
typedef struct
{
	...
	u8    Pull_Up_En; 				//上拉电阻使能位 			ENABLE,DISABLE
	u8    Pull_Down_En;				//下拉电阻使能位			ENABLE,DISABLE
	u8    Smit_Triggle_En;			//施密特触发使能位 			ENABLE,DISABLE
	u8    Fast_Switch_Speed_En;		//端口快速电平转换使能位	ENABLE,DISABLE
	u8    Strengthen_Current_En;	//增强驱动能力使能位		ENABLE,DISABLE
	u8    Digital_Input_En;			//数值信号输入使能位		ENABLE,DISABLE
	u8    User_Control_En;			//端口模式用户控制位		ENABLE,DISABLE
} GPIO_InitTypeDef;
